...
async def account_login(bot: Client, m: Message):
    ...
    try:
        ...
        for i in range(arg, len(links)):
            ...
            try:
                ...
                cc = f'{str(niru).zfill(3)}**.** {name1} {res}\n**Batch :-** {raw_text0}'
                cc1 = f'{str(niru).zfill(3)}**.** {name1} {res}.pdf\n**Batch :-** {raw_text0}'
                ...
                if cmd == "pdf" or ".pdf" in url or ".pdf" in name:
                    ...
                    thread = threading.Thread(target=async_helper, args=(url, cmd, name, bot, m, cc, thumb, prog))
                    thread.start()
    ...
    async def async_helper(url, cmd, name, bot, m, cc, thumb, prog):
       try:
