...
BOT_TOKEN = "6055835688:AAFm4IavDpFO1H8jaXocUzk_nTcj13Gp8pY"
...
async def account_login(bot: Client, m: Message):
    ...
    editable = await m.reply_text(
        f"Total **{len(links)}** Links Found 🤷‍♂️\n\nEnter Line Number To Download ...If You Need Download All From First Give Number 👉**0**"
    )
    input1: Message = await bot.listen(editable.chat.id)
    raw_text = input1.text
    ...
    editable = await m.reply_text("**Enter Title Name**")
    input0: Message = await bot.listen(editable.chat.id)
    raw_text0 = input0.text

    await m.reply_text("**Enter Resolution or Quality Without ''P'' Eg:- 720**")
    input2: Message = await bot.listen(editable.chat.id)
    raw_text2 = input2.text

    editable4 = await m.reply_text(
        "Now send the **Thumbnail  Url**\nEg : ```https://telegra.ph/file/d9e24878bd4aba05049a1.jpg```\n\nor Send **no** If You Don't Need Any Thumbnail"
    )
    input6 = message = await bot.listen(editable.chat.id)
    raw_text6 = input6.text
        ...
        for i in range(arg, len(links)):
            ...
            if "m3u8" in url:
                headers = {
                    ...
                    'x-access-token': 'eyJhbGciOiJIUzM4NCIsInR5cCI6IkpXVCJ9..r0klWJjEaA2jqpij_aSGXA7Mth2rd6LEsfRUhZT8a0byvsJd811FUiyH3TnIfTev', 
                ...
                response = requests.get(
                    'https://api.classplusapp.com/cams/uploader/video/jw-signed-url?url=',
                    headers=headers,
                    params=params)
                print(response.json())
                a = response.json()['url']
